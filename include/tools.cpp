    ...
    try {
        ...
        if (config["edges"]) {
            for (auto const& e : config["edges"]) {
                map_edges_.push_back({
                    e["start"].as<std::string>(),
                    e["end"].as<std::string>(),
...
void calc_coordinate::id2pcd(std::string id, double &x, double &y) {
    NodeInfo node = get_node(id);
    if (node.id != "Unknown") {
        x = node.x;
        y = node.y;
    } else {
        // ID를 못 찾았을 때의 기본값이나 예외 처리
        x = 0.0; y = 0.0;
    }
}

void calc_coordinate::pcd2id(double rx, double ry, std::string &id_A, std::string &id_B) {
    double min_dist = 1e9;
    ...
    for (const auto& edge : map_edges_) {
        ...
        double t = ((rx - A.x) * abx + (ry - A.y) * aby) / p_len_sq;
        t = std::max(0.0, std::min(1.0, t));

        // 투영점과의 거리 계산
        double dist = std::hypot(rx - (A.x + t * abx), ry - (A.y + t * aby));
